...
class NAF_STN_Block(nn.Module):
    def __init__(self, in_channel: int, in_resolution: int, num_naf: int, sampling=None):
        super().__init__()
...
class CoarseRestoration(nn.Module):
    def __init__(self):
        ...
        self.intro = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.outro = nn.Conv2d(in_channels=width, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
